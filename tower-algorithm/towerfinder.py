...
# Calculate pairwise distances between all districts and store in a DataFrame
distances = {}
...
# Create a distance matrix from the distances dictionary
...
# Fill any NaN values in distance_df with a small non-zero value to avoid invalid multiplications
distance_df = distance_df.replace(0, np.nan).fillna(1e-6)  # Small non-zero value

# Initialize the 'Towers Inside' column
df['Towers Inside'] = 0
# Define constants
total_destruction_rate = 100  # Since this represents the 100% total

# Function to calculate variables and prioritize districts for tower allocation
def calculate_variables(df, distance_df):
    # Set var1 as the district destruction rate
    df['var1'] = df['Destruction Rate']
    # Set var2 as total destruction rate (100% for this scenario)
    df['var2'] = total_destruction_rate
    # Calculate var3 as the ratio of total destruction rate to district destruction rate
    df['var3'] = df['var2'] / df['var1']
    # Set var4 as the population rate
    df['var4'] = df['Population']
    # Calculate var5 as the product of var3 and var4
    df['var5'] = df['var3'] * df['var4']
    
    # Calculate var6 by incorporating district distances and var5
    df['var6'] = [
        ((distance_df[district].sum() * var5 / (2 * distance_df.sum().sum())) if var5 and distance_df[district].sum() else 0) + var5
        for district, var5 in zip(df['District'], df['var5'].fillna(0))
    ]
    
    
    # Calculate var7 as var6 adjusted by the existing towers in each district
    df['var7'] = df['var6'] / (1 + df['Towers Inside'])

    #print("\n--- Debugging Info for Distribution ---")
...
def distribute_towers(df, num_towers, distance_df):
    ...
    while num_towers > 0:
        # Find the district with the highest priority for tower allocation
        highest_priority_index = df['var7'].idxmax()
...
# Define the number of towers to allocate
num_towers = 100
...
output_path = './distancemetric_output.csv'
